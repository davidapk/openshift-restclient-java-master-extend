...
import com.openshift.internal.restclient.model.volume.property.RbdVolumeProperties;
...
public class PersistentVolume extends KubernetesResource implements IPersistentVolume {
    ...
    private static final String PV_RBD = "spec.rbd";
    ...
    public IPersistentVolumeProperties getPersistentVolumeProperties() {
        ...
        case VolumeType.NFS:
            ...
            break;
        case VolumeType.RBD:
            properties = createRBDVolumeProperties();
            break;
        default:
            properties = null;
            break;
    ...
    private IRbdVolumeProperties createRBDVolumeProperties() {
        ModelNode node = get(PV_RBD);
        List<String> monitors = Arrays.asList(asString(node, MONITORS));
        String image = asString(node, IMAGE);
        String fsType = asString(node, FSTYPE);
        String pool = asString(node, POOL);
        String user = asString(node, USER);
        String keyring = asString(node, KEYRING);
        String secretref = asString(node, SECRETREF);
        return new RbdVolumeProperties(monitors, image, fsType, pool, user, keyring, secretref);
    }
